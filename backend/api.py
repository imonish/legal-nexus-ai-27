...
@app.get("/health")
def health():
    return {"status": "ok", "database": "legal_acts (1939-2019)"}
...
def query_endpoint(req: QueryRequest):
    ...
    # Guard for non-legal short queries
    legal_keywords = [
    ...
    if not is_legal and len(user_query.split()) < 4:
        return QueryResponse(
            question=user_query,
            answer="I'm a legal assistant specializing in Indian legislation (1939-2019). Please ask a legal question — for example: 'What are the penalties under the Insolvency and Bankruptcy Code?' or 'What are tenant rights in India?'",
            sources=[],
            timing={"embed_sec": 0, "retrieve_sec": 0, "generate_sec": 0},
        )

    try:
        result = ask(user_query, top_k=req.top_k, stream=False)
